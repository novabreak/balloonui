...
// ─────────────────────────────────────────────────────────────────
//
// 本控件是无窗口实现：它没有自己的 HWND，全部内容由 DUI 合成层绘制，因此它
// 是控件树里的普通一员。早先的输入框实现内嵌一个真的 Win32 输入框子窗口，
// 有以下四项限制，无窗口实现全部不再存在：
//
//   一、子窗口由系统绘制在 DUI 合成层之上，总是盖住同一位置的 DUI 内容，
//       绘制顺序无法调整。
//   二、子窗口不受父容器的裁剪区约束，超出父容器的部分依然可见，因此进不了
//       滚动容器 —— 滚出可视区之后仍会画出来。
//   三、子窗口的背景由系统填充，做不了圆角与半透明效果。
//   四、多行与密码这两项属性对应的是创建期固化的窗口风格位，运行期要改就得
//       销毁并重建整个子窗口。
//
// ─────────────────────────────────────────────────────────────────
// 通知时机：程序设值也会通知
