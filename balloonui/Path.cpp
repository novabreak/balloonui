...
tstring CPath::GetCurDir()
{
	...
	// _tcslen 的返回类型是 size_t，用 size_t 承接可避免 64 位下截断到 DWORD。
	// 该变量只在本函数内用于取末字符，不涉及任何跨文件的接口。
...
tstring CPath::GetTempPath()
{
	...
	// 同 GetCurDir：用 size_t 承接 _tcslen 的返回值，避免 64 位下截断到 DWORD。
...
tstring CPath::GetFileNameWithoutExtension(LPCTSTR lpszPath)
{
	...
	// rfind 的返回类型是 tstring::size_type，用它承接才能与 npos 直接比较；
	// 写法与下面的 GetExtension 保持一致。
